...
open_bfd(std::string &fname)
{
    ...
    if(!bfd_check_format(bfd_h, bfd_object)){
        fprintf(stderr, "file '%s' does not look like a executable (%s)\n", fname.c_str(), bfd_errmsg(bfd_get_error()));
        return NULL;
    }
    ...
    if(bfd_get_flavour(bfd_h) == bfd_target_unknown_flavour){
        fprintf(stderr, "unrecognized binary format for binary '%s' (%s)\n", fname.c_str(), bfd_errmsg(bfd_get_error()));
        return NULL;
    }
...
load_binary_bfd(std::string &fname, Binary *bin, Binary::BinaryType type)
{
    ...
    switch(bfd_info->mach)
    {
        ...
        case bfd_mach_x86_64:
            ...
            break;
        default:
            fprintf(stderr, "unsupported architechture (%s)\n", bfd_info->printable_name);
            goto fail;
    }
